...
with tabs[4]:
    ...
    if csv_file:
        ...
        if 'text' in df.columns:
            summaries = [summarizer_pipeline(article, max_length=150, min_length=50, do_sample=False)[0]["summary_text"] for article in df["text"]]
            st.subheader("Summarized Articles")
            for idx, summary in enumerate(summaries):
                st.write(f"Article {idx + 1}:")
                st.success(summary)  # Updated to ensure visibility
        else:
            st.error("CSV must contain a 'text' column with the articles.")
